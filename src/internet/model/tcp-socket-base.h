...
public:
  ...
  Ptr<TcpRxBuffer> GetRxBuffer (void) const;
    ...
    /**
   * \brief Get a pointer to the socket state
   * \return a pointer to the socket state object
   */
  Ptr<TcpSocketState> GetSocketState (void) const;
  ...
  /**
   * \brief Recalculate pacing timer based on current pacing rate
   */
  void RecalculatePacingTimer (void);
...
protected:
  ...
  /**
   * \brief Add INT Tags for the Socket
   * \param p Packet
   */
  void AddSocketIntTags (const Ptr<Packet> &p) const;
...
protected:
  ...
  bool     m_useTimestamp     {true}; //!< Actually use timestamp in calculating RTT
  uint32_t m_timestampToEcho  {0};    //!< Timestamp to echo

  bool m_intEnabled           {false};    //!< INT enabled
  uint32_t m_intQueueToEcho   {0};        //!< INT queue data to echo
  uint32_t m_intLinkToEcho    {0};        //!< INT link data to echo
  ...
  // Keep track of lastest send for pacing purposes 
  Time m_lastSendTime {Seconds (0.0)}; //!< Last time a packet was sent
