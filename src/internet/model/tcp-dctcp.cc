...
TcpDctcp::PktsAcked (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked, const Time &rtt)
{
  ...
  if (tcb->m_ecnState == TcpSocketState::ECN_ECE_RCVD ) //(tcb->m_ecnState == TcpSocketState::ECN_ECE_RCVD) tcb->m_lastRtt > 1.2 * tcb->m_minRtt tcb->m_lastIntQueue > 7 && 
    {
      ...
      if (m_ackedBytesTotal >  0)
        {
          ...
          // bytesEcn = tcb->m_lastQueue > 7  1000; // choosing some constant to normalize to that it is unlikely to ever reach
          // double link = tcb->m_lastLink; //problem is that this tops out at 100%, which is desirable
          // double both = 
        }
