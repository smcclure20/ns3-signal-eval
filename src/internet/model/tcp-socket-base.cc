...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddAttribute ("Timestamp", "Enable or disable Timestamp option",
                   ...
                   MakeBooleanChecker ())
    .AddAttribute ("TimestampForRtt", "Enable or disable using the timestamp for RTT measurements",
                   BooleanValue (true),
                   MakeBooleanAccessor (&TcpSocketBase::m_useTimestamp),
                   MakeBooleanChecker ())
    .AddAttribute ("EnableInt", "Enable or disable INT measurements",
                   BooleanValue (false),
                   MakeBooleanAccessor (&TcpSocketBase::m_intEnabled),
                   MakeBooleanChecker ())
    .AddAttribute ("MinRto",
...
TcpSocketBase::TcpSocketBase (const TcpSocketBase& sock)
  : TcpSocket (sock),
    ...
    m_useTimestamp (sock.m_useTimestamp),
    m_timestampToEcho (sock.m_timestampToEcho),
    m_intEnabled (sock.m_intEnabled),
    m_intQueueToEcho (sock.m_intQueueToEcho),
    m_intLinkToEcho (sock.m_intLinkToEcho),
...
TcpSocketBase::Send (Ptr<Packet> p, uint32_t flags)
{
  ...
  m_tcb->m_lastSendAttempt = Simulator::Now();
  if (m_state == ESTABLISHED || m_state == SYN_SENT || m_state == CLOSE_WAIT)
    {
      // Store the packet into Tx buffer
      if (!m_txBuffer->Add (p))
        { // TxBuffer overflow, send failed
          NS_LOG_DEBUG ("TX Buffer overflow");
          m_errno = ERROR_MSGSIZE;
          return -1;
        }
      if (m_shutdownSend)
        {
          NS_LOG_DEBUG ("Shutdown");
...
void 
TcpSocketBase::NotifyIdle(bool is_idle)
{
  NS_LOG_FUNCTION (this << is_idle);
  m_congestionControl->NotifyIdle(is_idle);
  if (is_idle)
  {
    return;
  }
  else
  {
    ...
    m_congestionControl->Reset(m_tcb, m_rtt);
  }
}
                            ...
                            const Address &toAddress)
{
  ...
  bool tagged = packet->RemovePacketTag(tag);
  if (tagged) 
  {
    NS_LOG_DEBUG("Packet is tagged (Q: " << tag.GetQueue() << " L: " << tag.GetLink() << ")");
    m_tcb->m_lastIntQueue = tag.GetQueue();
    m_tcb->m_lastIntLink = tag.GetLinkConverted();
    m_intQueueToEcho = tag.GetQueue();
    m_intLinkToEcho = tag.GetLink();
  }

  // Record Timestamp in tcb
  if (m_timestampEnabled && tcpHeader.HasOption (TcpOption::TS))
  {
    ...
    m_tcb->m_lastTimestampRtt = TcpOptionTS::ElapsedTimeFromTsValue (ts->GetEcho ());
    // m_tcb->m_lastTimestampRtt = NanoSeconds(ts->GetEcho ());
  }
      ...
      // of RecoveryPoint.
      ...
      else
        {
          ...
          if (exitedFastRecovery)
            {
              NewAck (ackNumber, true);
              if (m_congestionControl->ChangesCwndOnRto())
              {
                m_tcb->m_cWnd = m_tcb->m_ssThresh.Get ();
              }
              m_recoveryOps->ExitRecovery (m_tcb);
...
void
TcpSocketBase::AddSocketIntTags (const Ptr<Packet> &p) const
{
  if (m_intEnabled)
  {
    IntPacketTag intTag;
    intTag.SetQueue (m_intQueueToEcho);
    intTag.SetLink (m_intLinkToEcho);
    intTag.SetLock(false); // This is the line to change if turning on/off backward path TODO: Parameterize this
    p->AddPacketTag(intTag);
    NS_LOG_DEBUG("Adding tag: (Q: " << intTag.GetQueue() << " L: " << intTag.GetLink() << ")");
  }
}
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  NS_LOG_INFO ("Copied item: " << isStartOfTransmission);
  m_rateOps->SkbSent(outItem, isStartOfTransmission);
  NS_LOG_INFO ("More work");
  ...
  if (!m_history.empty ())
    {
      ...
      if (!h.retx && ackSeq >= (h.seq + SequenceNumber32 (h.count)))
        { // Ok to use this sample
          if (m_timestampEnabled && m_useTimestamp && tcpHeader.HasOption (TcpOption::TS))
            {
              Ptr<const TcpOptionTS> ts;
              ts = DynamicCast<const TcpOptionTS> (tcpHeader.GetOption (TcpOption::TS));
              m = TcpOptionTS::ElapsedTimeFromTsValue (ts->GetEcho ());
              // m = NanoSeconds(ts->GetEcho ());
              if (m.IsZero ())
                {
...
TcpSocketBase::AddOptionTimestamp (TcpHeader& header)
{
  ...
  // uint32_t delta = TcpOptionTS::NowToTsValue () - m_timestampToEcho;
...
Ptr<TcpSocketState>
TcpSocketBase::GetSocketState (void) const
{
  return m_tcb;
}
...
void 
TcpSocketBase::RecalculatePacingTimer (void) {
  NS_LOG_FUNCTION (this);
  if (m_tcb->m_pacing)
    {
      Time pacingInterval = m_tcb->m_pacingRate.Get ().CalculateBytesTxTime (m_tcb->m_segmentSize);
      if (m_lastSendTime + pacingInterval < Simulator::Now() + m_pacingTimer.GetDelayLeft()) {
        m_pacingTimer.Cancel ();
        // Time newDelay = m_lastSendTime + pacingInterval > Simulator::Now() ? pacingInterval - (Simulator::Now() - m_lastSendTime) : TimeStep(1);
        // m_pacingTimer.Schedule(newDelay);
        if (m_lastSendTime + pacingInterval > Simulator::Now()) {
          m_pacingTimer.Schedule(pacingInterval - (Simulator::Now() - m_lastSendTime) );
        }
      }
      else if (m_lastSendTime + pacingInterval > Simulator::Now() + m_pacingTimer.GetDelayLeft()) {
        m_pacingTimer.Cancel ();
        m_pacingTimer.Schedule(pacingInterval - (Simulator::Now() - m_lastSendTime) );
      }
    }
