...
public:
  ...
  /**
   * \brief Returns true if the CCA does not adjust the cwnd on RTO
   *
   */
  virtual bool ChangesCwndOnRto () const;

  /**
   * \brief Reset state of CCA
   *
   */
  virtual void Reset (Ptr<TcpSocketState> tcb, Ptr<RttEstimator> rtt);

  /**
   * \brief Notify the CCA that the flow is idle (only matters for Remy)
   *
   */
