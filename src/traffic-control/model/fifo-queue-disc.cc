...
TypeId FifoQueueDisc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::FifoQueueDisc")
    ...
    .AddAttribute ("EcnEnabled",
                "Enable ECN Marking",
                BooleanValue (false),
                MakeBooleanAccessor (&FifoQueueDisc::m_ecnEnabled),
                MakeBooleanChecker())
    .AddAttribute ("MarkEcnThreshold",
                "ECN marking threshold (RFC 8033 suggests 0.1 (i.e., 10%) default)",
                DoubleValue (50),
                MakeDoubleAccessor (&FifoQueueDisc::m_markEcnTh),
...
FifoQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
  ...
  if (m_ecnEnabled && ((int) GetCurrentSize ().GetValue()> (int) m_markEcnTh)) {
    NS_LOG_LOGIC ("Marking packet (ECN)");
    const char* ECN_MARK = "ECN threshold";
    Mark(item, ECN_MARK);
  }
