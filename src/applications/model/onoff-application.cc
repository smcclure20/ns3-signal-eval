...
OnOffApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::OnOffApplication")
    ...
    .AddAttribute ("MaxBytesDistribution",
                   "Distribution to sample max_bytes.",
                   StringValue ("ns3::ConstantRandomVariable[Constant=100.0]"),
                   MakePointerAccessor (&OnOffApplication::m_bytesDist),
                   MakePointerChecker <RandomVariableStream>())
    ...
    .AddAttribute ("EnableSeqTsSizeHeader",
                   ...
                   MakeBooleanChecker ())
    .AddAttribute ("EnableFlowTrace",
                   "Enable flow tagging for tracing packets.",
                   BooleanValue (false),
                   MakeBooleanAccessor (&OnOffApplication::m_traceFlow),
                   MakeBooleanChecker ())
    .AddAttribute ("FlowId",
                   "Flow tag to append to packets.",
                   UintegerValue (0),
    ...
    .AddTraceSource ("OnOff", "Application changes on/off state",
                     MakeTraceSourceAccessor (&OnOffApplication::m_isOn),
...
OnOffApplication::AssignStreams (int64_t stream)
{
  ...
  m_bytesDist->SetStream (stream + 2);
  return 3;
}
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_INFO("start at " << Simulator::Now().GetSeconds());
  m_isOn = true;
  m_lastStartTime = Simulator::Now ();
  m_socket->NotifyIdle(false);
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_INFO("Total bytes: " << m_totBytes << " max bytes: " << m_maxBytes);
  ...
  if (m_maxBytes == 0 || m_totBytes < m_maxBytes)
    {
      ...
      // NS_LOG_INFO("Scheduled sendpacket for " << nextTime.As(Time::S) << " seconds from now");
    }
  else
    { // All done, cancel any pending events
      // StopApplication ();
      m_totBytes = 0; // Need to sample the size of the maxBytes
      StopSending();
    }
}
...
{  // Schedules the event to start sending data (switch to the "On" state)
  ...
  // Set max bytes
  if (m_maxBytes != 0) {
    m_maxBytes = m_bytesDist->GetInteger();
    NS_LOG_INFO("max bytes sampled: " << m_maxBytes);
  }
}
...
{  // Schedules the event to stop sending data (switch to "Off" state)
  ...
  Time onInterval = MicroSeconds (m_onTime->GetValue ());
  if (m_maxBytes != 0) {
    onInterval = Seconds(15);
  }
  NS_LOG_LOGIC ("stop at " << onInterval.As (Time::S));
  NS_LOG_INFO("stop scheduled at " << onInterval.As (Time::S));
...
void OnOffApplication::SendPacket ()
{
  ...
  if ((unsigned) actual == m_pktSize)
  ...
  else
    {
      ...
      NS_LOG_INFO("Unable to send packet; actual " << actual << " size " << m_pktSize << "; caching for later attempt");
